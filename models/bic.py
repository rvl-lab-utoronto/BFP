...
class BiC(ContinualModel):
    ...
    def begin_task(self, dataset, t=0, start_epoch=0):
        ...
        if self.task_id > 0:
            ...
            self.net.train()
            self.lamda = 1 / (self.task_id+1)
            
            train_loader = dataset.train_loaders[self.task_id]
            icarl_replay(self, train_loader, val_set_split=self.args.valset_split)
        
        if hasattr(self, 'corr_factors'):
            del self.corr_factors

    def evaluate_bias(self, fprefx):
        resp = torch.zeros((self.task_id+1) * self.cpt).to(self.device)
        with torch.no_grad():
            with bn_track_stats(self, False):
                for data in self.val_loader:
                    ...
                    resp += self.forward(inputs, anticipate=fprefx == 'post')[:, :(self.task_id+1) * self.cpt].sum(0)
    ...
    def end_task(self, dataset):
        if self.task_id > 0:
            ...
            print("EVAL PRE", evaluate(self, dataset, self.task_id))
            ...
            for l in (range(self.args.bic_epochs) if self.args.non_verbose else tqdm(range(self.args.bic_epochs))):
                for data in self.val_loader:
                    ...
                    start_last_task = (self.task_id) * self.cpt
                    end_last_task = (self.task_id+1) * self.cpt
                    tout = out + 0
    ...
    def forward(self, x, anticipate=False):
        ...
        if ret.shape[0] > 0:
            if hasattr(self, 'corr_factors'):
                start_last_task = (self.task_id - 1 + (1 if anticipate else 0)) * self.cpt
                end_last_task = (self.task_id + (1 if anticipate else 0)) * self.cpt
    ...
    def observe(self, inputs, labels, not_aug_inputs):
        ...
        if self.task_id > 0:
            with torch.no_grad():
                ...
                if self.args.distill_after_bic:
                    if hasattr(self, 'old_corr'):
                        start_last_task = (self.task_id - 1) * self.cpt
                        end_last_task = (self.task_id) * self.cpt
            ...
            pi_hat = F.log_softmax(outputs[:,:self.task_id*self.cpt] / self.args.temp, dim=1)
            pi = F.softmax(old_outputs[:,:self.task_id*self.cpt] / self.args.temp, dim=1)

            dist_loss = -(pi_hat * pi).sum(1).mean()

        class_loss = self.loss(outputs[:,:(self.task_id+1)*self.cpt], labels, reduction='none')
    ...
    def build_buffer(self, dataset):
        ...
        examples_per_task = self.buffer.buffer_size // (self.task_id+1)
        ...
        counter = 0
        with torch.no_grad():
            for i, data in enumerate(dataset.train_loaders[self.task_id]):
                ...
                if examples_per_task - counter > 0:
                    self.buffer.add_data(examples=not_aug_inputs[:(examples_per_task - counter)],
                                     ...
                                     task_labels=(torch.ones(self.args.batch_size) *
                                                  (self.task_id))[:(examples_per_task - counter)])
