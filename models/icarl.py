...
def fill_buffer(self, mem_buffer: Buffer, dataset, t_idx: int) -> None:
        ...
        loader = dataset.train_loaders[t_idx]
        # Already change the dataloader to return normalized images
...
class ICarl(ContinualModel):
    ...
    def observe(self, inputs, labels, not_aug_inputs, logits=None):
        ...
        if not hasattr(self, 'classes_so_far'):
            ...
            self.register_buffer('classes_so_far', torch.cat((
                self.classes_so_far.to('cpu'), labels.to('cpu'))).unique())
        ...
        self.opt.zero_grad()
        loss = self.get_loss(inputs, labels, self.task_id, logits)
        loss.backward()
    ...
    def begin_task(self, dataset, t=0, start_epoch=0):
        ...
        icarl_replay(self, train_loader)

    def end_task(self, dataset) -> None:
        self.old_net = deepcopy(self.net.eval())
        self.net.train()
        with torch.no_grad():
            fill_buffer(self, self.buffer, dataset, self.task_id)
        self.class_means = None
